...
# 7. Tukey事后检验
print("\n===== Tukey事后检验：GDP =====")
tukey_gdp = pairwise_tukeyhsd(df_selected['gdp'], df_selected['country'], alpha=0.05)
print(tukey_gdp)

print("\n===== Tukey事后检验：人口 =====")
tukey_pop = pairwise_tukeyhsd(df_selected['population'], df_selected['country'], alpha=0.05)
print(tukey_pop)

print("\n===== Tukey事后检验：煤炭产量变化 =====")
tukey_coal = pairwise_tukeyhsd(df_selected['coal_prod_change_twh'], df_selected['country'], alpha=0.05)
print(tukey_coal)

# 正态性检验
print("\n===== Shapiro-Wilk 正态性检验（GDP模型残差） =====")
residuals_gdp = model_gdp.resid
shapiro_test = stats.shapiro(residuals_gdp)
print(f"统计量: {shapiro_test.statistic:.4f}, p值: {shapiro_test.pvalue:.4f}")
...
print(f"统计量: {levene_test.statistic:.4f}, p值: {levene_test.pvalue:.4f}")
